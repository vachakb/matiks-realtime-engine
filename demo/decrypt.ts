...
export interface MatiksRealtimeDecrypt {
  decryptQuestions(blobs: string[], key: string): Promise<Question[]>;
  /** Packed variant: ONE string in (blobs joined by '\n'), ONE string out (plaintexts
   *  joined by '\n'). Isolates per-element JSI marshaling cost from the off-thread AES. */
  decryptQuestionsPacked(packedBlobs: string, key: string): Promise<string>;
}
...
// ───────────────────────── Button C: off-thread + ONE JSI crossing each way ─────────────────────────

/**
 * Same off-thread decrypt as Button B, but the whole payload crosses the bridge as a SINGLE
 * string in each direction (blobs newline-joined in, plaintexts newline-joined out). If this
 * is dramatically faster than B and keeps the spinner alive, then per-element JSI marshaling of
 * the 75-string array — not the AES — was the residual JS-thread cost. Returns the decrypted
 * count (the demo doesn't render parsed content, so we skip JSON.parse, same as B).
 */
export async function decryptOffThreadPacked(
  blobs: string[],
  keyString: string,
): Promise<{ count: number; usedNative: boolean }> {
  if (nativeModule && typeof nativeModule.decryptQuestionsPacked === 'function') {
    const packedIn = blobs.join('\n');
    const packedOut = await nativeModule.decryptQuestionsPacked(packedIn, keyString);
    const count = packedOut.length === 0 ? 0 : packedOut.split('\n').length;
    return { count, usedNative: true };
  }
  return { count: 0, usedNative: false };
}
